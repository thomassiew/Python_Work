...
xpath2_functions.register_functions(lxml.etree, ns=None, functions=xpath2_functions.functions.ALL_FUNCTIONS.keys())
class autoprop(type):
...
class node(object):
	__metaclass__ = autoprop
	def __init__(self,xmlobj=None,encoding='utf-8',parser=lxml.etree.XMLParser(recover=True,remove_blank_text=True)):
		self.__obj=None
	...
	def cfgfile(pth=None,ext='.xml'):
		...
		if not tmp.exists():
			result=node("<root/>")
			dt={"Pid check":"True","Exit Error":"Exit With Error!","Exit":"Done!"}
  ...
  <item key="Exit">Done!</item>
	...
	def __convert__(obj):
		if isinstance(obj,list):
			rt=nodelist(map(lambda x: node.__convert__(x),obj))
	...
	def __getattr__(self,name,default=None):
		...
		return result
	def cscxml(self,i='item',k='key',*args,**kwds):
		print i,k,args,kwds
		gen=[]
		if args:
			gen+=args
	...
	def __str__(self):
		#if isinstance(self._obj,lxml.etree._Element):
		#	value=lxml.etree.tostring(self._obj,pretty_print=True,xml_declaration=False,encoding=self._encoding)
		#else:
		value=lxml.etree.tostring(self._obj,pretty_print=True,xml_declaration=False,encoding=self._encoding)
		return value.strip()
