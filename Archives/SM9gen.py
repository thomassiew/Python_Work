...
rules = Development.csc.csclxmls.node(rulebase)
...
def sm9_login_encryption():
    ...
    if os.path.isfile('creds.xml'):
        ...
        root = Development.csc.csclxmls.node("<root/>")
        user = root + 'user'
        item1 = user + 'item'
        item1['key'] = "username"
        item2 = user + 'item'
...
def sm9_login():
    ...
    try:
        ...
        credslogin = Development.csc.csclxmls.node(creds)
...
def sm9_navpanel(navigate):
    ...
    try:
        loadingpage = WebDriverWait(firefoxdriver.driver, 40).until(ec.presence_of_element_located((By.ID, navigate)))
        if loadingpage.is_displayed():
...
def sm9_search_id(ids):
    ...
    firefoxdriver.driver.find_element_by_xpath('//button[@aria-label="Start this Search (Ctrl+Shift+F6)"]').click()
...
def sm9_cancel():
    ...
    i = 1
    while i:
        inmhead = firefoxdriver.driver.find_elements_by_xpath('//iframe[contains(@title,"Update Incident Number")]')
        if len(inmhead):
...
def sm9_accept(ids):
    ...
    if abc is not "":
        ...
        firefoxdriver.driver.execute_script("arguments[0].setAttribute('value','%s')" % chosen_assignee, assignee)
